...
public class ExtNettyRoutingFilter extends NettyRoutingFilter {
  ...
  public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
    ...
    Flux<HttpClientResponse> responseFlux =
        client.request(method).uri(url).send((req, nettyOutbound) -> {
          ...
          return nettyOutbound.options(NettyPipeline.SendOptions::flushOnEach).send(request
              .getBody().map(dataBuffer -> ((NettyDataBuffer) dataBuffer).getNativeBuffer()));
          ...
          HttpHeaders filteredResponseHeaders =
              HttpHeadersFilter.filter(this.headersFilters.getIfAvailable(), headers, exchange,
    ...
    if (properties.getResponseTimeout() != null) {
      ...
      responseFlux =
          responseFlux.timeout(properties.getResponseTimeout(), Mono.error(new TimeoutException(
