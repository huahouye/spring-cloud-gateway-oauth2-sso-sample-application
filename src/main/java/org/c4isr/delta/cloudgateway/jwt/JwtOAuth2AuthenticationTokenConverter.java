...
public class JwtOAuth2AuthenticationTokenConverter
    implements Converter<Jwt, Mono<OAuth2AuthenticationToken>> {
  ...
  public Mono<OAuth2AuthenticationToken> convert(Jwt jwt) {
    ...
    OAuth2AuthenticationToken auth2AuthenticationToken = new OAuth2AuthenticationToken(
        jwtOAuth2User, jwtOAuth2User.getAuthorities(), (String) jwt.getClaims().get("client_id"));
